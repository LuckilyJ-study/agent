...
class QwenApiPlanner(JsonSchemaPlannerMixin):
    ...
    def __init__(
        ...
        self.response_format = (
            response_format
            or os.getenv("ROBOT_AGENT_API_RESPONSE_FORMAT", "auto")
        ).strip().lower()
        if self.response_format not in {
            ...
            raise PlannerServiceError(
                "ROBOT_AGENT_API_RESPONSE_FORMAT must be auto, json_schema, "
                "json_object, or text."
            )
        self._active_response_format = (
            "json_schema" if self.response_format == "auto" else self.response_format
        )
        self._response_format_resolved = self.response_format != "auto"
        if not self.api_key:
            raise PlannerServiceError(
    ...
    def _request_plan(self, user_message: str) -> list[PlanStep]:
        ...
        response = self._post_plan_json(payload, plan_schema)
        try:
    ...
    def _post_plan_json(
        ...
        formats = [self._active_response_format]
        if self.response_format == "auto" and not self._response_format_resolved:
        ...
        for index, format_name in enumerate(formats):
            ...
            try:
                ...
                can_fallback = (
                    ...
                    and not self._response_format_resolved
                    and index < len(formats) - 1
                    and "HTTP 400" in str(error)
                )
                if can_fallback:
                    continue
                raise

            self._active_response_format = format_name
            self._response_format_resolved = True
            return response

        raise PlannerServiceError("Qwen API did not accept a compatible JSON mode.")
