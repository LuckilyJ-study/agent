...
class StructuredActionMonitor:
    ...
    def before_action(
        ...
        if not observation.get("available", False):
            return (
                self._perception_failure(step, observation, "before_action")
    ...
    def during_action(
        ...
        if not observation.get("available", False):
            return (
                self._perception_failure(step, observation, "during_action")
                if self.require_perception
                else None
            )
        critical = self._critical_signal(step, observation)
    ...
    def verify(
        ...
        if not observation.get("available", False):
            if self.require_perception:
                return self._perception_failure(step, observation, "after_action")
            return {
                "success": True,
                "error_type": "NONE",
                "confidence": 1.0,
