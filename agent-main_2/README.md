...
YOLO-World-S 已通过独立 HTTP 服务接入第二个循环；未配置
`--vision-endpoint` 时仍使用占位感知，便于在没有摄像头和视觉环境的机器上
离线测试。当前 HTTP Provider 只提供二维检测和基础跟踪信号，不提供真机执行
所需的深度、手眼标定或 `robot_base` 三维坐标。
...
| Monitor | 支持 YOLO-World HTTP 感知，以及 `TARGET_LOST`、`GRASP_FAILED`、`OBJECT_DROPPED`、碰撞风险和硬件故障等结构化判断 |
...
- YOLO-World-S 二维检测与基础跟踪已通过 `YoloWorldHttpPerceptionProvider` 接入；高速多相机、深度、手眼标定和真实三维定位尚未接入。未指定视觉服务时默认使用 `NullPerceptionProvider`。
- 当前 YOLO HTTP Provider 明确声明 `hardware_ready=False` 且只支持 `bbox_2d`，因此不能绕过真机模式对 `robot_base_xyz` 的要求。
...
## 接入 YOLO-World-S Monitor 服务

YOLO-World 使用独立环境和进程运行，Agent 不直接加载其 300 MB 权重，而是通过
`YoloWorldHttpPerceptionProvider` 调用 `/health`、`/configure` 和 `/observe`。
先启动 `yolo-world-monitor/yolo_world_service.py`，确认服务可用：

```powershell
Invoke-RestMethod http://127.0.0.1:8765/health
```

再从 `agent-main_2` 运行不调用 Qwen 的视觉端到端测试：
...
python .\run_agent_simulation.py `
  --planner scripted `
  --task "Put dough on tray" `
  --pick-failures 0 `
  --vision-endpoint "http://127.0.0.1:8765" `
  --vision-min-confidence 0.15 `
  --vision-disable-gripper-tracking `
  --vision-target-alias "dough=pizza dough" `
  --vision-target-alias "tray=pizza tray"
```

`--vision-disable-gripper-tracking` 只适用于普通摄像头中没有机械臂夹爪的 DryRun
联调。连接真实机械臂后应保留夹爪检测，并根据现场数据重新设置阈值。

`--vision-target-alias LOGICAL=PROMPT` 可以重复使用。左侧是 Agent 计划中的逻辑
目标，右侧是更适合 YOLO-World 开放词汇检测的提示词。Provider 会同时配置
`tray` 和 `pizza tray`；视觉服务返回 `pizza tray` 时仍会生成 `entity_id=tray`，
因此 Monitor 可以按 Agent 的逻辑目标匹配。别名不会绕过置信度和目标可见性检查。
测试前应让摄像头中同时清晰出现面团和托盘；没有满足阈值的检测时，程序返回
`TARGET_LOST` 并停止 DryRun 流程，这是预期的失败关闭行为。

每个技能执行前，Agent 会把当前 target、可信别名和可选的 `robot gripper`
发送给视觉服务，并等待至少 3 帧新配置结果。显式配置视觉服务后，如果服务在
启动时不可用，程序返回退出码 2；如果执行期间服务不可用，Monitor 返回
`PERCEPTION_FAILED`，不会退回假视觉结果。

Linux 服务器上的 Agent 侧命令等价：

```bash
PYTHONPATH=src python run_agent_simulation.py \
  --planner scripted \
  --task "Put dough on tray" \
  --pick-failures 0 \
  --vision-endpoint "http://127.0.0.1:8765" \
  --vision-min-confidence 0.25 \
  --vision-target-alias "dough=pizza dough" \
  --vision-target-alias "tray=pizza tray"
```

这里仍然是 `DryRunRobotController`。当前 YOLO Provider 只有二维框，不能设置
`hardware_ready=True`；真机目标动作还必须接入深度和手眼标定，输出新鲜的
`position_xyz_m` 与 `coordinate_frame="robot_base"` 后才能通过
`PhysicalTargetGate`。
