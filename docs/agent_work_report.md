...
| 感知接口 | `PerceptionProvider`、`hardware_ready`/目标配置/定位模式契约、无摄像头占位实现、脚本化测试实现 | 高速相机、YOLO-World-S、跟踪、三维定位和坐标变换 |
| 真机动作前门禁 | `PhysicalTargetGate` 检查新鲜检测、bbox 与 `robot_base` XYZ，并在 Router 前 fail-closed | 用真实相机与标定数据验证门禁参数 |
| Monitor | `StructuredActionMonitor` 规则、动作前/中/后检查 | YOLO-World-S 模型本体及真实检测流 |
...
未来 YOLO-World-S Provider 可以根据当前步骤动态设置文字标签，例如：
...
> `StructuredActionMonitor` 已实现，但 YOLO-World-S 模型本体尚未接入。当前代码不会自行加载 YOLO-World-S，也不会把普通模拟数据冒充真实摄像头结果。

无相机模式使用 `NullPerceptionProvider`，明确返回 `available=false`。测试可以使用 `ScriptedPerceptionProvider` 注入确定性的帧序列。
...
### 5.1 YOLO-World-S 和高速相机

需要实现真实的 `PerceptionProvider`，负责：
...
尚未完成的是物理世界一侧：YOLO-World-S 模型本体、高速相机与跟踪、三维定位和标定、真实机械臂 Controller、真实训练 Policy 以及最终物理目标验证。因此，当前系统适合做架构开发、Planner 评估、技能注册、调度恢复和模拟测试；在完成真机接入及安全验证前，不能把 dry-run 的 `completed` 当作真实任务完成。
